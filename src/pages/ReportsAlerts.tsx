  ...
  const handleDownloadReport = async (report: typeof reports[0]) => {
    try {
      ...
      if (report.type === 'monthly') {
        // Fetch stations and recent data
        const stationsResponse = await axios.get(`${API_BASE_URL}/stations`);
        const stations = stationsResponse.data.stations || [];
        
        reportContent = `
MONTHLY GROUNDWATER ASSESSMENT REPORT
Generated: ${new Date().toLocaleString()}
${'-'.repeat(80)}

EXECUTIVE SUMMARY
${'-'.repeat(80)}
Total Monitoring Stations: ${stations.length}
Active Stations: ${stations.filter((s: any) => s.status === 'active').length}
Report Period: ${new Date().toLocaleString('en-US', { month: 'long', year: 'numeric' })}

STATION OVERVIEW
${'-'.repeat(80)}
${stations.slice(0, 20).map((station: any, index: number) => `
${index + 1}. ${station.name}
   Location: ${station.location}
   Status: ${station.status}
   Water Level: ${station.lastReading ? station.lastReading + 'm' : 'N/A'}
   Data Points: ${station.dataPoints || 0}
   Battery: ${station.battery_level || 100}%
`).join('\n')}

OBSERVATIONS
${'-'.repeat(80)}
- Regular monitoring continues across all ${stations.length} stations
- Water level data collected from ${stations.filter((s: any) => s.lastReading).length} active stations
- System operational status: Excellent

RECOMMENDATIONS
${'-'.repeat(80)}
- Continue regular monitoring schedule
- Review stations with low battery levels
- Schedule maintenance for inactive stations

Report generated by AquaMonitor System
${'-'.repeat(80)}
        `;
...
Active Stations: ${stations.filter((s: any) => s.status === 'active').length}
Warning Status: ${stations.filter((s: any) => s.status === 'warning').length}
Inactive Stations: ${stations.filter((s: any) => s.status === 'inactive').length}
...
Station ${index + 1}: ${station.name}
  Code: ${station.id}
  Status: ${station.status}
  Data Points Collected: ${station.dataPoints || 0}
  Battery Level: ${station.battery_level || 100}%
  Last Reading: ${station.lastReading ? station.lastReading + 'm' : 'N/A'}
  Last Update: ${station.lastReadingTime || 'N/A'}
`).join('\n')}

MAINTENANCE RECOMMENDATIONS
${'-'.repeat(80)}
${stations.filter((s: any) => s.battery_level < 30).map((s: any) => 
  `- ${s.name}: Battery low (${s.battery_level}%) - Schedule battery replacement`
).join('\n') || '- All stations have adequate battery levels'}
...
This is a system-generated report from AquaMonitor.
For detailed analysis, please access the dashboard.

Report Type: ${report.type}
Generated By: AquaMonitor System
${'-'.repeat(80)}
        `;
      }
