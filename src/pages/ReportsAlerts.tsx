...
const ReportsAlerts: React.FC = () => {
  ...
  const handleDownloadReport = async (report: typeof reports[0]) => {
    try {
      // Fetch data from API
      const stationsResponse = await axios.get(`${API_BASE_URL}/stations`);
      const stations = stationsResponse.data.stations || [];
      const alertsResponse = await axios.get(`${API_BASE_URL}/dashboard/alerts?limit=10`);
      ...
      doc.text(report.title.toUpperCase(), pageWidth / 2, yPosition, { align: 'center' });
      
      yPosition += 10;
      doc.setFontSize(10);
      doc.setTextColor(100);
      doc.text(`Generated: ${new Date().toLocaleString()}`, pageWidth / 2, yPosition, { align: 'center' });
      doc.text(`Report Type: ${report.type}`, pageWidth / 2, yPosition + 5, { align: 'center' });
      ...
      const warningStations = stations.filter((s: any) => s.status === 'warning').length;
      ...
      const summaryData = [
        ['Metric', 'Value'],
        ['Total Monitoring Stations', stations.length.toString()],
        ['Active Stations', `${activeStations} (${((activeStations/stations.length)*100).toFixed(1)}%)`],
        ['Warning Status', `${warningStations} stations`],
        ['Inactive Stations', `${inactiveStations} stations`],
        ['Recent Alerts', recentAlerts.length.toString()],
        ['Report Period', new Date().toLocaleString('en-US', { month: 'long', year: 'numeric' })]
      ];
      ...
      // Add chart visualization (simulated as table with visual indicators)
      doc.setFontSize(14);
      doc.setTextColor(0);
      doc.text('WATER LEVEL DISTRIBUTION', 15, yPosition);
      yPosition += 8;

      // Group stations by water level ranges
      const levelRanges = [
        { range: '0-5m', count: stations.filter((s: any) => s.lastReading >= 0 && s.lastReading < 5).length, color: [220, 38, 38] },
        { range: '5-10m', count: stations.filter((s: any) => s.lastReading >= 5 && s.lastReading < 10).length, color: [251, 146, 60] },
        { range: '10-15m', count: stations.filter((s: any) => s.lastReading >= 10 && s.lastReading < 15).length, color: [250, 204, 21] },
        { range: '15-20m', count: stations.filter((s: any) => s.lastReading >= 15 && s.lastReading < 20).length, color: [74, 222, 128] },
        { range: '20m+', count: stations.filter((s: any) => s.lastReading >= 20).length, color: [34, 197, 94] }
      ];

      const chartData = [
        ['Water Level Range', 'Station Count', 'Percentage', 'Status Indicator']
      ];
      
      levelRanges.forEach(range => {
        const percentage = ((range.count / stations.length) * 100).toFixed(1);
        const bars = '█'.repeat(Math.floor(range.count / 5)) || '▫';
        chartData.push([range.range, range.count.toString(), `${percentage}%`, bars]);
      });
      ...
      doc.setFontSize(14);
      doc.setTextColor(0);
      doc.text('STATION STATUS OVERVIEW', 15, yPosition);
      yPosition += 8;
      ...
      const statusTypes = [
        { status: 'Active', count: activeStations, color: [34, 197, 94] },
        { status: 'Warning', count: warningStations, color: [251, 146, 60] },
        { status: 'Inactive', count: inactiveStations, color: [220, 38, 38] }
      ];

      statusTypes.forEach(type => {
        const percentage = ((type.count / stations.length) * 100).toFixed(1);
        const bars = '█'.repeat(Math.floor(type.count / 3)) || '▫';
        statusData.push([type.status, type.count.toString(), `${percentage}%`, bars]);
      });
      ...
      doc.setFontSize(14);
      doc.setTextColor(0);
      doc.text('DETAILED STATION INFORMATION', 15, yPosition);
      yPosition += 8;

      const stationTableData = [
        ['Station Name', 'Location', 'Status', 'Water Level', 'Battery', 'Data Points']
      ];

      stations.slice(0, 30).forEach((station: any) => {
        stationTableData.push([
          station.name,
          station.location,
          station.status,
          station.lastReading ? `${station.lastReading}m` : 'N/A',
          `${station.battery_level || 100}%`,
          (station.dataPoints || 0).toString()
        ]);
      });

      autoTable(doc, {
        startY: yPosition,
        head: [stationTableData[0]],
        body: stationTableData.slice(1),
        theme: 'striped',
      ...
      doc.setFontSize(14);
      doc.setTextColor(0);
      doc.text('RECENT ALERTS', 15, yPosition);
      yPosition += 8;

      const alertsTableData = [
        ['Type', 'Message', 'Location', 'Time']
      ];

      recentAlerts.slice(0, 10).forEach((alert: any) => {
      ...
      autoTable(doc, {
        startY: yPosition,
        head: [alertsTableData[0]],
        body: alertsTableData.slice(1),
        theme: 'grid',
        headStyles: { fillColor: [0, 102, 204], textColor: 255, fontSize: 9 },
        bodyStyles: { fontSize: 8 },
        columnStyles: {
      ...
      if (inactiveStations > 0) {
        recommendations.push(`• Schedule maintenance for ${inactiveStations} inactive station(s)`);
      }
      
      const lowBatteryStations = stations.filter((s: any) => s.battery_level < 30);
      if (lowBatteryStations.length > 0) {
        recommendations.push(`• Replace batteries for ${lowBatteryStations.length} station(s) with low battery levels`);
      }
      
      if (warningStations > 0) {
        recommendations.push(`• Investigate ${warningStations} station(s) showing warning status`);
      }
      
      recommendations.push('• Continue regular monitoring schedule across all stations');
      recommendations.push('• Review water level trends for seasonal patterns');
      ...
      doc.text('Report generated by AquaMonitor System', pageWidth / 2, yPosition, { align: 'center' });
      doc.text(`Page 1 of ${doc.getNumberOfPages()}`, pageWidth / 2, yPosition + 5, { align: 'center' });
